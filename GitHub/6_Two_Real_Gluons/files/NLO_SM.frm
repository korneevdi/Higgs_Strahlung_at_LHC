...
f  LepCurr, WqqbVertex, Q2prop, gqqbarVertex, gqqbarLeftVertex, gqqbarRightVertex, gammaGluonL, gammaGluonR, gamma, Bra, Ket, Ket1, Bra2;                         *** Non-commuting functions
...
*** EXPRESSION FROM FEYNMAN DIAGRAMS
G [iM.LLL] = - Bra2(j2,-1) * WqqbVertex(mu) * Q2prop(j1,j3) * gqqbarLeftVertex(j3,r3) * Ket1(j1,-1) * Wprop(q,mu,nu) * WHVertex(nu,sigma) * Wprop(k,sigma,rho) * LepCurr(j4,j5,rho)
             + Bra2(j2,-1) * gqqbarLeftVertex(j3,r3) * Q2prop(j2,j3) * WqqbVertex(mu) * Ket1(j1,-1) * Wprop(q,mu,nu) * WHVertex(nu,sigma) * Wprop(k,sigma,rho) * LepCurr(j4,j5,rho); 

G [iM.LLR] = - Bra2(j2,-1) * WqqbVertex(mu) * Q2prop(j1,j3) * gqqbarRightVertex(j3,r3) * Ket1(j1,-1) * Wprop(q,mu,nu) * WHVertex(nu,sigma) * Wprop(k,sigma,rho) * LepCurr(j4,j5,rho)
             + Bra2(j2,-1) * gqqbarRightVertex(j3,r3) * Q2prop(j2,j3) * WqqbVertex(mu) * Ket1(j1,-1) * Wprop(q,mu,nu) * WHVertex(nu,sigma) * Wprop(k,sigma,rho) * LepCurr(j4,j5,rho); 
...
Bracket i_, Mw, [v^-1], gw, gs, Vud, Tg3, sqrt2, [denWprop^-1];
